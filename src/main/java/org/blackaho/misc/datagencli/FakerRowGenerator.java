...
public class FakerRowGenerator implements RowGenerator {
  ...
  protected List<FieldCacheEntry> fieldCache;
  ...
  public List generateRow(Long lineNumber) {
    initFieldCache();
    return fieldCache.stream().map( fieldCacheEntry -> this.getFieldValue(fieldCacheEntry, lineNumber) )
  ...
  public String generateRowLine(Long lineNumber) {
    initFieldCache();
    return fieldCache.stream().map( fieldCacheEntry -> this.getFieldValue(fieldCacheEntry, lineNumber).toString() )
        .collect(Collectors.joining(this.separator));
  }

  protected void initFieldCache() {
    if (fieldCache == null) {
      fieldCache = fields.stream().map(this::createFieldCacheEntry).collect(Collectors.toList());
    }
  }


  static class FieldCacheEntry {
    String field;
    ...
    public FieldCacheEntry(String field, EntryType entryType, Object object, Method method, Object[] args) {
      this.field = field;
  ...
  /**
   * Creates an FieldCacheEntry based on string with field definition. This FieldCacheEntry can be passed to
   * getFieldValue() to generate a value for the field.
   * Parses the field definition string (including any arguments) passed as parameter, maps them to
   * corresponding Faker methods and return a FieldCacheEntry with that..
   ...
   * @return a FieldCacheEntry object that can be used with getFieldValue()
   */
  protected FieldCacheEntry createFieldCacheEntry(String field) {
    try {
      ...
      if (RANDOMSTRING_REGEXP.matcher(field).find()) {
        ...
        return new FieldCacheEntry(field, FieldCacheEntry.EntryType.FAKER_METHOD, faker,
        ...
        return new FieldCacheEntry(field, FieldCacheEntry.EntryType.FAKER_METHOD, faker.number(),
        ...
        return new FieldCacheEntry(field, FieldCacheEntry.EntryType.SEQUENCE,null, null,
            new Object[] { Long.parseLong(m.group(1)), Long.parseLong(m.group(2)) } );
      } else if (field.equals("rowNumber")) {
        return new FieldCacheEntry(field, FieldCacheEntry.EntryType.ROWNUMBER,null, null, null);
      } else {
        ...
        return new FieldCacheEntry(field, FieldCacheEntry.EntryType.FAKER_METHOD, objToCall, methodToCall, new Object[] {} );
      }
    } catch (Exception ex) {
      return new FieldCacheEntry(field, FieldCacheEntry.EntryType.FAKER_METHOD, null, null, null);
    }
  }

  /**
   * Get a value for a field using reflection on Faker object, based on method/params defined on fieldCacheEntry.
   * E.g., field "name.firstName" will call faker.name().firstName() and return its value.
   * @param fieldCacheEntry object, created with createFieldCacheEntry()
   * @return value for corresponding field
   */
  protected Object getFieldValue(FieldCacheEntry fieldCacheEntry, Long lineNumber) {
    // handles stuff we can handle directly, w/o faker, line rowNumber and sequence
    if (fieldCacheEntry.entryType == FieldCacheEntry.EntryType.ROWNUMBER) {
      return lineNumber;
    } else if (fieldCacheEntry.entryType == FieldCacheEntry.EntryType.SEQUENCE) {
      return (long) fieldCacheEntry.generatorArguments[0] + ((lineNumber-1L) * (long) fieldCacheEntry.generatorArguments[1]);
    } else { // FieldCacheEntry.EntryType.SEQUENCE
